...
const MyPortfolioImageDetail = ({ image, tagString }) => {
    ...
    return (
        <div className='image-container portfolio-image'
            key={`${image.id}-artist-image`}>
            <img onClick={() => setShowModal(true)}
                className='grid-image portfolio-grid-image'
                id={image.id}
                key={image.id}
                src={image.imageUrl}
            ...
            {isPortfolioPage &&
                <div className='portfolio-image-info'>
                    <div className='tag-container'>
                        <div
                            className='portfolio-tags tag-text'
                            key={image}>Tags: <p
                                className='single-tag portfolio-tag tag-text'>
                                {tagString ? tagString.split(',').map(tag => `${tag}`)
                                    : 'No tags'}</p>
                        </div>
                    </div>
